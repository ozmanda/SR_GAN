...
class PhIREGANs:
    ...
    DEFAULT_N_EPOCHS_PRETRAIN = 10  # Number of epochs of pretraining
    DEFAULT_N_EPOCHS_TRAIN = 10  # Number of epochs of training
    ...
    def __init__(self, data_type, N_epochs_pretrain=None, N_epochs_train=None, learning_rate=None, epoch_shift=None, save_every=None,
                 print_every=None, mu_sig=None):

        self.N_epochs_pretrain = N_epochs_pretrain if N_epochs_pretrain is not None else self.DEFAULT_N_EPOCHS_PRETRAIN
        self.N_epochs_train = N_epochs_train if N_epochs_train is not None else self.DEFAULT_N_EPOCHS_TRAIN
    ...
    def pretrain(self, r, data_path, model_path, pretrainedmodel_path=None, batch_size=100):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            # Start training
            iters = 0
            for epoch in range(self.epoch_shift + 1, self.epoch_shift + self.N_epochs_pretrain + 1):
                print('Epoch: %d' % (epoch))
                start_time = time()
    ...
    def train(self, r, data_path, trainedmodelpath, model_path, batch_size=100, alpha_advers=0.001):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            # Start training
            iters = 0
            for epoch in range(self.epoch_shift + 1, self.epoch_shift + self.N_epochs_train + 1):
                ...
                try:
                    ...
                    while True:
                        ...
                        if (iters % self.print_every) == 0:
                            ...
                            print(
                                'Number of generator training steps=%d, Number of discriminator training steps=%d, ' % (
                                gen_count, dis_count))
                            print('G loss=%.5f, Content component=%.5f, Adversarial component=%.5f' % (
                            gl, np.mean(g_cl), np.mean(g_al)))
                            print('D loss=%.5f' % (dl))
    ...
    def test(self, r, data_path, model_path, batch_size=100, plot_data=False):
        ...
        print('Done.')
        return data_out, self.data_out_path
    ...
    def _parse_train_(self, serialized_example, mu_sig=None):
        ...
        feature = {'index': tf.compat.v1.FixedLenFeature([], tf.int64),
                   ...
                   'data_HR': tf.compat.v1.FixedLenFeature([], tf.string),
