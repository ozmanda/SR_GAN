...
class SR_NETWORK(object):
    ...
    def generator(self, x, r, is_training=False, reuse=False):
        ...
        with tf.compat.v1.variable_scope('generator', reuse=reuse):
            ...
            for i in range(16):
                ...
                with tf.compat.v1.variable_scope('block_{}a'.format(i+1)):
                    x = deconv_layer_2d(x, [k, k, C_out, C_in], output_shape, stride, k)
                    x = tf.nn.relu(x)

                with tf.compat.v1.variable_scope('block_{}b'.format(i+1)):
                    x = deconv_layer_2d(x, [k, k, C_out, C_in], output_shape, stride, k)
            ...
            for i, r_i in enumerate(r):
                C_out = (r_i**2)*C_in
                with tf.compat.v1.variable_scope('deconv{}'.format(i+3)):
                    output_shape = [N, r_prod*h+2*k, r_prod*w+2*k, C_out]
                    x = deconv_layer_2d(x, [k, k, C_out, C_in], output_shape, stride, k)
                    x = tf.compat.v1.depth_to_space(x, r_i)
                    x = tf.nn.relu(x)

                r_prod *= r_i

            output_shape = [N, r_prod*h+2*k, r_prod*w+2*k, C]
            with tf.compat.v1.variable_scope('deconv_out'):
    ...
    def discriminator(self, x, reuse=False):
        ...
        with tf.compat.v1.variable_scope('discriminator', reuse=reuse):
            ...
            x = flatten_layer(x)
            with tf.compat.v1.variable_scope('fully_connected1'):
                x = dense_layer(x, 1024)
                x = tf.nn.leaky_relu(x, alpha=0.2)

            with tf.compat.v1.variable_scope('fully_connected2'):
                x = dense_layer(x, 1)
